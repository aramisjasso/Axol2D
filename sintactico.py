...
def p_restoExpresionAritmetica(p):
...
def p_operadorAdicion(p):
    '''operadorAdicion : MAS
                       | MENOS'''
    p[0] = p[1]

#<operadorMultiplicacion> ::= POR | DIVISION | MODULO
def p_operadorMultiplicacion(p):
...
def p_expresionLogicaConjuncion(p):
...
def p_restoExpresionComparacion(p):
...
def p_declaracionEstructuraDatos(p):
...
def p_declaracionArregloSimple(p):
...
def p_declaracionMatrizSimple(p):
